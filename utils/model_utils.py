...
def get_model_saved_from_local(model_id, device):
    pruned_dict = torch.load(model_id, map_location=device)
    tokenizer, model = pruned_dict['tokenizer'], pruned_dict['model']
    return model, tokenizer

def get_MoE_model_from_local(device, model_name, model_load_path):
    if "OLMoE" in model_name:
        from transformers import OlmoeForCausalLM, AutoTokenizer, BitsAndBytesConfig
        # 选择是否启用量化
        quantization_chioce = 0 # 设置为1以启用4位量化
...
def parse_order(order):
    ...
    if isinstance(order, list):
        out = [str(x).strip().upper() for x in order]
    else:
    ...
    if not out:
        raise ValueError(f"Invalid order={order}, should contain P/M/D, e.g. 'PMD' or 'MD'.")
    return out
